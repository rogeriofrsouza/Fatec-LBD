...
ALTER TABLE autor_livro ADD CONSTRAINT fk_id_livro FOREIGN KEY (id_livro) REFERENCES livro(id_livro);
ALTER TABLE autor_livro ADD CONSTRAINT fk_id_autor FOREIGN KEY (id_autor) REFERENCES autor(id_autor);

/* Criando FK para assunto */
ALTER TABLE livro ADD CONSTRAINT fk_id_assunto FOREIGN KEY (id_assunto) REFERENCES assunto;
...
/* 4- Alterar a coluna titulo da tabela Livros de 30 para 45 posições. */
...
DELETE FROM livro WHERE id_assunto = 10 
AND TO_CHAR(data_edicao, 'YYYY') < '1980';

DELETE FROM livro WHERE id_assunto = 10 
AND TO_CHAR(data_edicao, 'mm/YYYY') < '05/1980';

DELETE FROM livro WHERE id_assunto = 10 
AND extract(year FROM data_edicao) < '1999';
